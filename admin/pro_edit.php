        ...
        <div class="col-md-9 col-lg-9">
          <form method="post" enctype="multipart/form-data">
             <?php
                         ...
                         $Fur_pr_row = mysqli_fetch_array($Fur_pr_run);
                          ...
                          if(isset($_POST['update'])){ 
                            ...
                            if(!empty($title) || !empty($size) || !empty($price) || !empty($status) || !empty($category) || !empty($detail) || !empty($image)){
                              ...
                              $query = "UPDATE product SET title='$title', category=$category, size='$size', price =$price, detail='$detail', image='$image', status='$status' Where pid=$Fur_pro_id";                            
                                if(mysqli_query($con,$query)){
                                    $path = "img/".$image;
                                    if(move_uploaded_file($tmp_image,$path) == true){
                                        copy($path,"../".$path);                                                                    
                                    }
                                    header("location:pro_edit.php?pid=$Fur_pro_id");
              ...
              <div class="row">
                <div class="col-md-12">
                <label for="size">Product Detail :</label>
                  <input name="detail" value="<?php echo $db_detail; ?>" >
